...
.3}{RFC8445 Section-4.4.3} with the following modification. The content that is signed is:\\
    ...
    \begin{center}
        \begin{tabular}{l}
            $C_{peer} = \text{DH}(E_{serv}^{pub},\, E_{peer}^{priv})$\vspace{0.5em}
            \\
            $C_{serv} = \text{DH}(E_{peer}^{pub},\, E_{serv}^{priv})$\vspace{0.5em}
            \\
    ...
    \begin{center}
        \begin{tabular}{l}
            data := AEAD($T_{m}^{send}$, $N_{m}^{send}$, $P$, $\epsilon$)\vspace{0.5em} \\
    ...
    \begin{center}
        \begin{tabular}{l}
            $G$ := session-i  \vspace{0.5em}                                                              \\
            $H$ := peer-id  \vspace{0.5em}                                                                \\
            $J$ := peer-key  \vspace{0.5em}                                                               \\
            $(V_{peer}^{send} = T_{serv}^{recv}, P_{peer}^{recv} = P_{serv}^{send}) := \text{KDF}(\text{HASH}(G\,
            ||\, H\,||\, J), \epsilon)$  \vspace{0.5em}                                                   \\
            $M_{peer}^{send} = M_{serv}^{recv} = M_{peer}^{recv} = M_{serv}^{send} := 0$ \vspace{0.5em} \\
        \end{tabular}
    \end{center}
    ...
    \begin{center}
        \begin{tabular}{l}
            data := AEAD($V_{m}^{send}$, $M_{m}^{send}$, $P$, $\epsilon$)\vspace{0.5em} \\
            counter := $M_{m}^{send}$
