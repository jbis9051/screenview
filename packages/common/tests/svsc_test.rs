...
fn test_lease_response_accepted(){
    ...
    assert_eq!(&message.response_data.as_ref().unwrap().id, &b"idid".as_slice().read_u32::<LittleEndian>().unwrap(), "id");
    assert_eq!(&message.response_data.as_ref().unwrap().cookie, b"cookiecookiecookiecookie", "cookie");
    test_write(&message, bytes);
}
